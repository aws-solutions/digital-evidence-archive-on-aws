...
describe('cognito helpers integration test', () => {
    ...
    it('should fail when SSM params do not exist', async () => {
        ...
        expect(getTokenPayload(idToken, fakeRegion)).rejects.toThrow("Unable to grab the parameters in SSM needed for token verification.");
    });
    ...
    it('should fail if token is modified', async () => {
        ...
        const modifiedToken = token.substring(0, replacementIndex) + replacementChar + token.substring(replacementIndex + 1);
    ...
    it('should fail when first/last name not in id token', async () => {
        ...
        expect(getDeaUserFromToken(tokenPayload)).rejects.toThrow("First and/or last name not given in id token.");
